...
function Menu() {
    ...
    this.options = ["normal", "hard", "genocide","aprilfools"];

    this.select_text = new PIXI.extras.BitmapText("なんいどをせんたく", {font: "28px tusb"});
    this.select_text.position.set(80, 260);
    this.select_text.visible = false;

    this.normal_text = new PIXI.extras.BitmapText("", {font: "30px tusb"});
    this.normal_text.position.set(80, 300);
    this.normal_text_chars = 0;
    this.hard_text = new PIXI.extras.BitmapText("", { font: "30px tusb"});
    this.hard_text.position.set(80, 332);
    this.hard_text_chars = 0;
    this.genocide_text = new PIXI.extras.BitmapText("", { font: "30px tusb"});
    this.genocide_text.position.set(80, 364);
    this.genocide_text_chars = 0;

    this.easy_mode_enabled = false;

    this.normal_text_text = "ノーマル";
    this.hard_text_text = "ハード";
    this.genocide_text_text = "ジェノサイド";
...
Menu.prototype.moveUp = function() {
    ...
    if (this.easy_mode_enabled == false && this.current_option == 3) {
        this.current_option = 2;
    }
    if (this.easy_mode_enabled == true && this.current_option == 0) {
        this.current_option = 2;
    }
